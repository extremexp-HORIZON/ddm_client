...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="ddm-uploader-meta-delete", description="Delete uploader metadata for a file")
    ...
    resp = client.uploader_metadata.delete(file_id)

    if client.storage and not args.no_store:
        # after delete, store the (now empty) current state if backend returns it,
        ...
        append_log(client, project_id, file_id, action="uploader_metadata.delete", ok=True)
