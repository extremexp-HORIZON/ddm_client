...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="ddm-uploader-meta-update", description="Update uploader metadata for a file")
    ap.add_argument("--project_id", required=True)
    ap.add_argument("--file_id", "--file-id", required=True, dest="file_id")
    ap.add_argument("--json", default=None, help='Inline JSON object, e.g. \'{"sensor":"A2"}\'')
    ...
    resp = client.uploader_metadata.update(file_id, body)
    ...
    if client.storage and not args.no_store:
        ...
        append_log(client, project_id, file_id, action="uploader_metadata.update", ok=True, details={"set": meta_obj})
