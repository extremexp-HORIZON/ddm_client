...
def _csv_list(v: Optional[str]) -> Optional[list[str]]:
    """
    Accepts:
      --project_id projectA/sub1
      --project_id projectA/sub1,projectB
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="ddm-catalog-list", description="Catalog list")
    ...
    ap.add_argument("--store_project", default=None, help="Where to store under projects/<id>/... (optional). If omitted, no project-scoped storage.")
    ...
    # Optional project-scoped storage/logging (only if you provide --store_project)
    if args.store_project:
        ...
        append_project_log(client, pid, action="catalog_list", ok=True, details={"saved": saved})

    #print(json.dumps(out, indent=2, ensure_ascii=False))
    print ("Files listed:", len(out.get("items", [])))
    return 0
