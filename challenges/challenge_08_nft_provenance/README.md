# Challenge 08 — NFT Provenance (Sepolia)
...
- In **MetaMask → NFTs**, you should see an **NFT** representing your Dataset Request 📦✨
---
...
✅ After success:
- The script prints a `dataset fingerprint` — copy it (needed for Step 4).
...
✅ In that case: **wait for the human validators** to validate your dataset.  
(They usually check at least once per day ⏳)
...
- The `NFT` is linked to the validation `IPFS results` and proves completion of the tutorial 🎉
...
- When you see a notification that validations are completed ✅, proceed to **Step 5 (Claim rewards)**.
