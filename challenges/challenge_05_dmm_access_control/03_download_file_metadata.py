...
def main(argv: list[str] | None = None) -> int:
    ...
    # dict: file_id -> metadata dict
    m = client.file_metadata.get_many(file_ids)
    ...
    if args.out:
        ...
        saved_to = client.storage.write_json(
            key,
            {"project_id": project_id, "file_ids": file_ids, "metadata": m},
        )

    print(
        json.dumps(
            {"ok": True, "project_id": project_id, "file_ids": file_ids, "saved_to": saved_to, "metadata": m},
            indent=2,
            ensure_ascii=False,
        )
    )
