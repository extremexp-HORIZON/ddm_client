...
def _to_html_bytes(x: Union[str, bytes, bytearray, BytesIO]) -> bytes:
    # normalize whatever the SDK returns into bytes
    if isinstance(x, str):
...
def main(argv: list[str] | None = None) -> int:
    ...
    html_obj = client.file_metadata.get_report_html(file_id)
    html_bytes = _to_html_bytes(html_obj)
    ...
    # 2) storage (write_bytes expects bytes, not BytesIO)
        ...
        append_file_log(
            ...
            action="report_html",
            ok=True,
            details={"saved_to": saved_to, "bytes": len(html_bytes)},
        )
    ...
    print(
        json.dumps(
            {"ok": True, "project_id": project_id, "file_id": file_id, "saved_to": saved_to, "bytes": len(html_bytes)},
            indent=2,
        )
    )
    return 0
